...
def mock_async_session():
    with patch('controllers.friendship_controller.async_session', new_callable=MagicMock) as mock_sessionmaker:
...
def test_accept_request(client):
    ...
    with patch("controllers.friendship_controller.accept_request", new_callable=AsyncMock) as mock_accept:
...
def test_reject_request(client):
    request_id = str(uuid4())
    with patch("controllers.friendship_controller.reject_request", new_callable=AsyncMock) as mock_reject:
...
def test_get_friends(client):
    user_id = str(uuid4())
    with patch("controllers.friendship_controller.get_friends", new_callable=AsyncMock) as mock_get:
...
def test_get_pending_requests(client):
    user_id = str(uuid4())
    with patch("controllers.friendship_controller.get_pending_requests", new_callable=AsyncMock) as mock_get:
...
def test_send_request_success(client, mock_async_session):
    ...
    with patch("controllers.friendship_controller.uuid.uuid4", return_value=expected_uuid) as mock_uuid_gen:

        mock_async_session.execute = AsyncMock()

        mock_async_session.execute.return_value = AsyncMock()
        mock_async_session.execute.return_value.scalar_one_or_none.return_value = None 

        response = client.post(
            "/friendships/send", json={"sender_id": sender_id, "receiver_id": receiver_id}
        )
...
def test_send_request_already_exists(client, mock_async_session):
    ...
    mock_async_session.execute.return_value = AsyncMock() 
    mock_async_session.execute.return_value.scalar_one_or_none.return_value = mock_existing_friendship 
